...
void carrera_auto(int id, int distancia) {
    ...
    while (posicion < distancia) { // Genera un avance aleatorio entre 1 y 10 metros
...

    {
   	// Cuando el auto termina, se almacena su ID en el vector de posiciones finales
        ...
        std::cout << "Auto" << id << " termina la carrera en el lugar " << posiciones.size() << "!" << std::endl;
    }
}
...
int main(int argc, char* argv[]) {
    ...
    int N = std::atoi(argv[2]); // Número de autos(hilos)
    ...
    // Obtiene el maximo de hebras de la CPU
    unsigned int max_hebras = std::thread::hardware_concurrency();

    if (max_hebras == 0) {
        std::cerr << "No se pudo determinar la cantidad máxima de hebras soportadas por el hardware." << std::endl;
        return 1;
    }

    if (N > static_cast<int>(max_hebras)) {
        std::cerr << "El número de autos (hebras) excede la cantidad máxima de " << max_hebras << " hebras soportadas por la CPU." << std::endl;
        return 1;
    }
